...
class Playwright
  ...
  def self.find_by_name(name)
    PlayDBConnection.instance.execute(<<-SQL, name)
      SELECT playwrights.* FROM plays JOIN playwrights ON plays.playwright_id = playwrights.id
      WHERE playwrights.name = ?;
    SQL
  end

  def initialize(name, year) #(this is the initialize method)
    @id = nil
    @name = name
    @year = year
  end
  ...
  def get_plays(name)
    PlayDBConnection.instance.execute(<<-SQL, @name)
      SELECT * FROM plays JOIN playwrights ON playwrights.id= plays.playwright_id WHERE playwrights.name = ?;
    SQL
  end
